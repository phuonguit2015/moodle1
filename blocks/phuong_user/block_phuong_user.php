...
class block_phuong_user extends block_base {
    function init() {
        $this->title = get_string('pluginname','block_phuong_user');
    }
    ...
    function  get_content()
    {
        ...
        //QRcode::png(''.$USER->firstname.$USER->lastname.'-'.$USER->id.'-'.$USER->username);
        /*$this->content->text = "<div class=\"info\"><img src='".$CFG->wwwroot."/blocks/phuong_user/QRCode.php'>"
                .$USER->firstname.$USER->lastname.'-'.$USER->id.'-'.$USER->username."</div>"
                . "<script src='http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js'>
</script>
<script type='text/javascript'>
 $(document).ready(function(){
 $('#test11').val($(location).attr('href'));
  }
  );
</script>
...
<input id='test11' type='text'>";*/
        $this->content->text = '<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js">
</script>
<script type="text/javascript">
 $(document).ready(function(){
pshowImgQRCode($(location).attr("href"));
  }
  );
</script>
<script>
function pshowImgQRCode(str) {
 $.ajax({ 
        type: "GET", 
        url:"'.$CFG->wwwroot.'/blocks/phuong_user/ajax_rec_code.php?link=" + str, 
        data: { get_param: "value" }, 
       dataType: "json",
    success: function (data) { 
        $.each(data, function(index, element) {
if (index=="imgsrc") {
document.getElementById("imgsrc").src = element;
}else {
document.getElementById("rec_link").innerHTML= element;
}
...
<div id="txtHint">                                
<img id="imgsrc"/>
<span id="rec_link"></span>
</div>';
        return $this->content;
    }
    //http://localhost:8080/moodle/rec_code/3?redirect=http://localhost:8080/moodle/my/index.php?sesskey=ntd3xX2thB   
    //src="http://localhost:8080/moodle/blocks/phuong_user/QRCode.php?link=http://localhost:8080/moodle/my/index.php?sesskey=ntd3xX2thB"
    /*function get_content() {
        ...
        if (isset($CFG->block_phuong_user_timetosee)) {
            $timetoshowusers = $CFG->block_phuong_user_timetosee * 60;
        }
        $now = time();
        ...
        if (!has_capability('block/phuong_user:viewlist', $this->page->context)) {
            if (!$usercount = $DB->count_records_sql($csql, $params)) {
                $usercount = get_string("none");
            }
            $this->content->text = "<div class=\"info\">".get_string("periodnminutes","block_phuong_user",$minutes).": $usercount</div>";
            return $this->content;
        }
        ...
        $this->content->text = "<div class=\"info\">".$OUTPUT->user_picture($user, array('size'=>40, 'alttext'=>false, 'link'=>false))
                .$USER->firstname.$USER->lastname.'-'.$USER->id.'-'.$USER->username."$usercount</div>";
        
        //QRcode::png('PHP QR Code :)');
        
        // echo '<img src="example_001_simple_png_output.php" />';
        /*$this->content->text = "<div class=\"info\">(".get_string("periodnminutes","block_phuong_user",$minutes).$USER->id."$usercount)</div>";
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                if (isguestuser($user)) {
                    ...
                    $this->content->text .= $OUTPUT->user_picture($user, array('size'=>16, 'alttext'=>false, 'link'=>false)) .$USER->username."   ".$USER->fullname.$USER->id.'</a></div>';
                }
